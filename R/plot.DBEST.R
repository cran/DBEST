...
function(x, figure=1, ...) {
        ...
        if(figure==1) {
                ...
                fit_zoo <- as.zoo(ts(x$Fit, start=as.Date(time(x$Data)[1]), frequency=frequency(x$Data)))
                ...
                par(mar=c(8,5,0,3))
                f_local_zoo <- as.zoo(ts(x$f_local, start=as.Date(time(x$Data)[1]), frequency=frequency(x$Data)))
